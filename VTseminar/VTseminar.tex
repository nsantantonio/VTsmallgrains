...
\frame[label=always]{\frametitle{A traditional breeding program}
...
% \frame{\frametitle{$21^\text{st}$ Century Plant Breeding Pipeline}
\frame[label=always]{\frametitle{Increasing population size increases intensity}
% High throughput phenotypes serve \highlite{2} purposes for breeding:
...
\frame[label=always]{\frametitle{Current model for genomic prediction works...}
  \begin{columns}
...
\frame[label=always]{\frametitle{... but can we do better?}
...
\frame[label=always]{\frametitle{Sparse testing is (almost) uniformly superior to unobserved prediction}
...
\frame[label=always]{\frametitle{Rapid cycle recurrent truncation beats traditional, at first}
\begin{columns}
...
\frame[label=always]{\frametitle{Parent selection must balance mean and variance}
\vspace{-1cm}
  \begin{columns}
    \begin{column}{0.5\linewidth}
      \centering
...
\frame[label=always]{\frametitle{Optimal contributions of parents}
% From Akdemir et al. (2019):
...
\frame[label=always]{\frametitle{Optimal contributions can maintain $V_g$ at the cost of short-term gains}
\begin{columns}
...
\frame[label=always]{\frametitle{Can we have our cake and eat it too?}
\begin{columns}
\begin{column}{0.05\linewidth}
\end{column}
\begin{column}{0.45\linewidth}
...
\frame[label=always]{%\frametitle{Not without foundational capabilities!}
...
\frame[label=always]{\frametitle{Resources: part of a breeding informatics ecosystem}
% \hspace{-1cm}
\begin{tikzpicture}
\small
...
\subsection{Quantitative Genetics}
\frame[label=always]{\frametitle{Introducing complex ideas through historical perspectives}
...
\frame[label=always]{\frametitle{Genetic Variance}
  \begin{tabular}{l}
...
\frame[label=always]{\frametitle{Additive only}
   \centering
   \begin{columns}
   \begin{column}{0.5\linewidth}
...
\frame[label=always]{\frametitle{Additive with Dominance}
   \centering
   \begin{columns}
   \begin{column}{0.5\linewidth}
...
\frame[label=always]{\frametitle{Using simulation as a teaching tool}
...
\frame[label=always]{\frametitle{Genomic Prediction of Grain Yield}
\begin{columns}
\begin{column}{0.6\linewidth}
\centering
...
\frame[label=always]{\frametitle{Recognition}
\begin{columns}
